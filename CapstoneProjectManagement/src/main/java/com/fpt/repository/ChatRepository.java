...
public interface ChatRepository extends JpaRepository<Chat, Integer> {
    ...
    @Query(value = "SELECT p.id id, p.title title, cd.read_status readStatus, c.type type FROM posts p INNER JOIN chat c ON p.id = (SELECT SUBSTRING_INDEX(c.room, '_', -1)) INNER JOIN chat_details cd ON c.id = cd.chat_id WHERE cd.user_id = ?1 GROUP BY p.id, p.title, cd.read_status, c.type", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT id, title, readStatus,type FROM (SELECT \n" +
            ...
            "GROUP BY c.`room`,\n" +
            "  cd.read_status,\n" +
            "  c.`user_id`) tmp", nativeQuery = true)
