...
public interface ChatRepository extends JpaRepository<Chat,Integer> {
    ...
    @Query(value = "SELECT p.id id, p.title title, cd.read_status readStatus FROM posts p INNER JOIN chat c ON p.id = (SELECT SUBSTRING_INDEX(c.room, '_', -1)) INNER JOIN chat_details cd ON c.id = cd.chat_id WHERE cd.user_id = ?1 GROUP BY p.id, p.title, cd.read_status", nativeQuery = true)
    List<ChatDTO> findChatsByUserId(String userId);

    @Query(value = "SELECT c.`room` id, (SELECT us.user_name FROM users us WHERE us.id = c.`user_id`) title, cd.read_status readStatus FROM users u INNER JOIN chat c ON u.id = c.`user_id` INNER JOIN chat_details cd ON c.id = cd.chat_id WHERE cd.user_id = ?1 AND c.`user_id` != ?1 AND c.type = 'private' GROUP BY c.`room`, cd.read_status", nativeQuery = true)
