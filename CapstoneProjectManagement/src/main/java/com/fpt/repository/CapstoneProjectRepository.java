...
public interface CapstoneProjectRepository extends JpaRepository<CapstoneProjects, Integer>{
	...
	@Query(value = "SELECT ru.*,st.name as nameStatus, count(de.id) as countDetail FROM CapstoneProject.capstone_projects ru LEFT JOIN CapstoneProject.capstone_project_details de ON de.capstone_project_id = ru.id LEFT JOIN CapstoneProject.status st ON st.id = ru.status_id WHERE (de.user_id = ?1 OR ?1 = '-1' ) AND (ru.status_id = ?4 OR ?4 = -1 ) AND (ru.profession_id = ?5 OR ?5 = -1 ) AND ru.name like ?6  group by ru.id,ru.description_action,ru.description,ru.document,ru.name,ru.name_abbreviation,ru.name_lang_other,ru.name_vi,ru.program,ru.specialty,ru.profession_id,ru.status_id,st.name LIMIT ?2,?3", nativeQuery = true)
